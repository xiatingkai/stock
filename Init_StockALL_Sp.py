...
if __name__ == '__main__':
    ...
    ts.set_token('d17aadec99cccf8952e9931142525c6088f6e87ce71d385df3e865b4')
    pro = ts.pro_api()
    ...
    # 建立数据库连接,剔除已入库的部分
    db = pymysql.connect(host='localhost', user='root', passwd='xtk@sjtu', db='stock', charset='utf8')
    cursor = db.cursor()
    cursor.execute("create table if not exists stock_all( \
        state_dt CHAR(16), stock_code CHAR(16), open FLOAT ,close FLOAT, high FLOAT,low FLOAT,\
        vol INT, amount FLOAT, pre_close FLOAT, amt_change FLOAT, pct_change FLOAT)")
    # 设定需要获取数据的股票池
    # stock_pool = ['603912.SH','300666.SZ','300618.SZ','002049.SZ','300672.SZ']
    stock_pool = ['600221.SH','601398.SH']
